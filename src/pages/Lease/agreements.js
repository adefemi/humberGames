...
function Agreements(props) {
  ...
  const formatTemplate = (templates, app, lease_id) => {
    const newTemplate = [];
    templates.map(item => {
      if (item.user_reference === "all") {
        newTemplate.push({
          lease_id,
          title: item.title,
          content: item.content
            .replace(/all/g, `${app.user.first_name} ${app.user.last_name}`)
            .replace(/all/g, getPropOwnerDetails(app)),
  ...
  const addClause = () => {
    setTemplates([
      ...leaseTemplates,
      { user_reference: "all", lease_id: leaseId }
    ]);
  };
  const onChange = (id, e) => {
