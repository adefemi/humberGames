...
function LandlordConv(props) {
  ...
  const formatTemplate = (templates, app, lease_id) => {
    const newTemplate = [];
    templates.map(item => {
      if (item.user_reference === "lesser") {
        newTemplate.push({
          lease_id,
          title: item.title,
          content: item.content
            .replace(/lesser/g, `${app.user.first_name} ${app.user.last_name}`)
  ...
  const addClause = () => {
    setTemplates([
      ...leaseTemplates,
      { user_reference: "lesser", lease_id: leaseId }
    ]);
  };
  const onChange = (id, e) => {
  ...
  return (
    <div className="lease-card covenant">
      <h3>Landlord Covenants</h3>
      {props.mode === 1 &&
        (leaseTemplates.length < 1 ? (
          <p>No Landlord Covenant defined</p>
        ) : (
